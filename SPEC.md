...
Encoded variants are generated by the encoding engine (see below) into subdirectories:
```
payloads/
├── full.txt                    # Raw
├── math.txt
...
├── oob.txt
├── url-encoded/
│   ├── full.txt                # URL-encoded versions
│   ├── math.txt
│   ├── error.txt
│   ├── timing.txt
│   └── oob.txt
├── json-safe/
│   ├── full.txt                # JSON string-safe (escaped quotes, backslashes, control chars)
│   ├── math.txt
│   ├── error.txt
│   ├── timing.txt
│   └── oob.txt
├── xml-safe/
│   ├── full.txt                # XML-safe (escaped <, >, &, ', ")
│   ├── math.txt
│   ├── error.txt
│   ├── timing.txt
│   └── oob.txt
├── double-url-encoded/
│   ├── full.txt                # Double URL-encoded
│   ├── ...
├── html-entity-encoded/
│   ├── full.txt                # HTML entity encoded (&lt; &#x27; etc.)
│   ├── ...
├── unicode-escaped/
│   ├── full.txt                # \uXXXX escaped
│   ├── ...
├── base64/
│   ├── full.txt                # Base64 encoded (for deserialization, JWT, data: URIs)
│   ├── ...
└── hex-escaped/
    ├── full.txt                # \xXX escaped
    └── ...
```
...
The encoding engine is a script that takes the 5 raw pillar lists and produces encoded variants for every delivery context automatically. **You write payloads once in raw form. The engine generates all encoded versions.**

### Why An Encoding Engine?

Currently, `json-safe-polyglots.txt` is manually maintained alongside `polyglots.txt`. This means:
- Every new payload must be added to multiple files
- Encoding errors creep in when done by hand
- Adding a new encoding format means duplicating all lists again

The engine eliminates this. Raw lists are the single source of truth. Encoded variants are **generated artifacts**, not manually maintained.
